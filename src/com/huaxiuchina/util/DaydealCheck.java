...
public class DaydealCheck {
	...
	public void daydealTemp2(String filename, String name) throws Exception {
		...
		for (int i = 1; i < list.size() / 7; i++) {
			...
			if (listTemp.size() == 7) {
				DaydealDao daydealDao = new DaydealDao();
				// 实体化
				Daydeal daydeal1 = new Daydeal(String.valueOf((int) Double
						.parseDouble(listTemp.get(2).toString())), listTemp
						.get(1).toString(), listTemp.get(3).toString(),
						String.valueOf((int) Double.parseDouble(listTemp.get(4)
	...
	public void daydealTemp3(String filename, String name) throws Exception {
		...
		for (int i = 1; i < list.size() / 11; i++) {
			...
			if (listTemp.size() == 11) {
				DaydealDao daydealDao = new DaydealDao();
				// 实体化
				System.out.println(listTemp.get(1).toString());
				Daydeal daydeal1 = new Daydeal(listTemp.get(1).toString(), listTemp
						.get(2).toString(), listTemp.get(3).toString(),
	...
	public void daydealTemp4(String filename, String name) throws Exception {
		...
		for (int i = 1; i < list.size() / 9; i++) {
			...
			if (listTemp.size() == 9) {
				DaydealDao daydealDao = new DaydealDao();
				// 实体化
				Daydeal daydeal1 = new Daydeal((listTemp.get(2).toString()).substring(1,(listTemp.get(2).toString()).length()-1),
						(listTemp.get(3).toString()).substring(1,(listTemp.get(3).toString()).length()-1), (listTemp.get(4).toString()).substring(1,(listTemp.get(4).toString()).length()-1),
						String.valueOf((int) Double.parseDouble(listTemp.get(5)
								.toString())), String.valueOf((int) Double
								.parseDouble(listTemp.get(6).toString())),
						new GetDate().getDate(), null, name, null, null);
	...
	public static void main(String[] args) throws Exception {
		new DaydealCheck().isType("E:/3.xlsx", "3", "test3");
		String a="\"2\"";
